 ...
 * 整型转换器
 */
public class LongConverterImpl implements DefaultDataConverter<Long>, ConverterConstants {

    @Override
    public boolean accept(ConverterItemEntity item) {
        return ValidHelper.isEquals(item.getType(), DATA_TYPE_LONG);
    }
    ...
    public Object validation(ConverterItemEntity item, Object o) {
        ...
        if (ValidHelper.isNotBlank(max)) {
            AssertHelper.EX_VALIDATION.isTrue(v <= DataHelper.getLong(max),
    ...
    @Override
    public Long convert(ConverterItemEntity item, Object o) {
        return o == null ? null : DataHelper.getLong(o);
    }
}
