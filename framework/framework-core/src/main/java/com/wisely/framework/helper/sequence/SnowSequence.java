...
/**
 * com.dounion.core.sequence.SnowSequence
 * <p>
...
//         this.workerId = calcWorkerId(dataCenterId, maxWorkerId);
    ...
    public SnowSequence(long dataCenterId, long workerId) {
        AssertHelper.EX_VALIDATION.isTrue(workerId >= 0 && workerId <= maxWorkerId,
                String.format("Worker Id must be between 0 and %d", maxWorkerId));
        AssertHelper.EX_VALIDATION.isTrue(dataCenterId >= 0 && dataCenterId <= maxDataCenterId,
        ...
        if (timestamp < lastTimestamp) {
            ...
            if (offset <= 5) { // 闰秒
                try {
                    ...
                    throw SystemException.builder(e, "some error found in id generator");
                }
            } else { // 发生了时间回拨
                throwClockException(offset);
            }
    ...
    public class TNode {
        ...
        public String buildTimeId() {
            ...
            return localDateTime.format(formatter) + StringHelper
