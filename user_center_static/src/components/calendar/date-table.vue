...
export default {
    ...
    methods: {
        formatLunarDayName(date) {
            const date2 = moment(date).format('YYYY-MM-DD');
            const ymd = date2.split('-');
            const lunar = LunarCalendar.solarToLunar(ymd[0], ymd[1], ymd[2]);
            return lunar;
        },
        toNestedArr(days) {
        ...
        getCellClass({ text, type }) {
            const classes = ['el-calendar-table__td', type];
            if (type === 'current') {
        ...
        cellRenderProxy({ text, type }) {
            ...
            const lunar = this.formatLunarDayName(date);
            return render({ date, data, lunar });
        }
    },
    ...
    computed: {
        ...
        rows() {
            ...
            // let tag = true;
            // while (tag) {
            //     const index = days.length - 7;
            //     if (days[index]['type'] === 'next') {
